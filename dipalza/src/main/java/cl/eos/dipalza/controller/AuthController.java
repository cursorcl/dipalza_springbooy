...
public class AuthController {
	...
	@Value("${security.jwt.refresh-days}")
	long refreshDays;

	public record LoginReq(String username, String password) {
	}

	public record TokenRes(String accessToken, String refreshToken, long expiresInSeconds, VendedorDTO vendedor) {
	}
	...
	public TokenRes login(@RequestBody LoginReq req) {
		...
		return new TokenRes(access, refreshRaw, 60L * 10, vendedorDto); // 10 min si así configuraste
	}
	...
	public TokenRes refresh(@RequestBody RefreshReq req) {
		...
		var tokens = rtRepo.findAll(); // optimiza con consulta; aquí ejemplo simple
		...
		return new TokenRes(access, newRefreshRaw, 60L * 10, vendedorDto);
	}
}

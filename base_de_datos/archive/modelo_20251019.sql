...
CREATE TABLE app_role (
	...
	CONSTRAINT PK__app_role__3213E83FDFD86E49 PRIMARY KEY (id),
	CONSTRAINT UQ__app_role__72E12F1BA02DEC54 UNIQUE (name)
);
...
CREATE TABLE app_user (
	...
	created_at date DEFAULT CAST(SYSUTCDATETIME() AS DATE) NOT NULL,
	updated_at date DEFAULT CAST(SYSUTCDATETIME() AS DATE) NOT NULL,
	CONSTRAINT PK__app_user__3213E83F7459B955 PRIMARY KEY (id),
	CONSTRAINT UQ__app_user__F3DBC572B4D6807B UNIQUE (username)
);
...
CREATE TABLE producto (
	...
	last_update date DEFAULT CAST(SYSUTCDATETIME() AS DATE) NOT NULL,
...
CREATE TABLE app_refresh_token (
	...
	created_at date DEFAULT CAST(SYSUTCDATETIME() AS DATE) NOT NULL,
	CONSTRAINT PK__app_refr__3213E83F04E9AA5A PRIMARY KEY (id),
	CONSTRAINT UQ__app_refr__80488ABFE301C05E UNIQUE (user_id,token_hash),
	CONSTRAINT FK__app_refre__user___748F2482 FOREIGN KEY (user_id) REFERENCES app_user(id)
);
...
CREATE TABLE app_user_roles (
	...
	CONSTRAINT FK__app_user___role___70BE939E FOREIGN KEY (role_id) REFERENCES app_role(id),
	CONSTRAINT FK__app_user___user___6FCA6F65 FOREIGN KEY (user_id) REFERENCES app_user(id)
);
...
CREATE TABLE numerados (
	id bigint IDENTITY(1,1) NOT NULL,
	producto_id varchar(255) COLLATE Modern_Spanish_CI_AS NOT NULL,
	numero int NOT NULL,
	peso decimal(19,4) NOT NULL,
	estado char(1) COLLATE Modern_Spanish_CI_AS DEFAULT 'D' NOT NULL,
	creado_en date DEFAULT CAST(SYSUTCDATETIME() AS DATE) NOT NULL,
	actualizado_en date DEFAULT CAST(SYSUTCDATETIME() AS DATE) NOT NULL,
	CONSTRAINT PK_InventarioNumerados PRIMARY KEY (id),
	CONSTRAINT FK_InvNum_producto FOREIGN KEY (producto_id) REFERENCES producto(Articulo)
);
...
CREATE TABLE venta (
	...
	CONSTRAINT venta_cliente_FK FOREIGN KEY (rut_cliente,codigo_cliente) REFERENCES cliente(rut,codigo) ON DELETE CASCADE ON UPDATE CASCADE,
	CONSTRAINT venta_condicionventa_FK FOREIGN KEY (condicion_venta) REFERENCES condicionventa(codigo) ON DELETE CASCADE ON UPDATE CASCADE,
	CONSTRAINT venta_ruta_FK FOREIGN KEY (codigo_ruta) REFERENCES ruta(codigo),
	CONSTRAINT venta_vendedor_FK FOREIGN KEY (codigo_vendedor,tipo_vendedor) REFERENCES vendedor(codigo,tipo)
);
...
CREATE TABLE venta_detalle (
	...
	piezas int DEFAULT 0 NOT NULL,
	CONSTRAINT PK_venta_detalle PRIMARY KEY (venta_id,linea),
	CONSTRAINT FK_vd_producto FOREIGN KEY (producto_id) REFERENCES producto(Articulo),
	CONSTRAINT FK_vd_venta FOREIGN KEY (venta_id) REFERENCES venta(id)
);
...
CREATE TABLE venta_detalle_pieza (
	id bigint IDENTITY(1,1) NOT NULL,
	venta_id bigint NOT NULL,
	linea int NOT NULL,
	...
	CONSTRAINT FK_vdp_inv FOREIGN KEY (inv_id) REFERENCES numerados(id),
	CONSTRAINT FK_vdp_vd FOREIGN KEY (venta_id,linea) REFERENCES venta_detalle(venta_id,linea)
);
 CREATE NONCLUSTERED INDEX IX_vdp_venta_linea ON ventas.dbo.venta_detalle_pieza (  venta_id ASC  , linea ASC  )  
	 WITH (  PAD_INDEX = OFF ,FILLFACTOR = 100  ,SORT_IN_TEMPDB = OFF , IGNORE_DUP_KEY = OFF , STATISTICS_NORECOMPUTE = OFF , ONLINE = OFF , ALLOW_ROW_LOCKS = ON , ALLOW_PAGE_LOCKS = ON  )
	 ON [PRIMARY ] ;
 CREATE NONCLUSTERED INDEX UX_vdp_inv ON ventas.dbo.venta_detalle_pieza (  inv_id ASC  )  
	 WITH (  PAD_INDEX = OFF ,FILLFACTOR = 100  ,SORT_IN_TEMPDB = OFF , IGNORE_DUP_KEY = OFF , STATISTICS_NORECOMPUTE = OFF , ONLINE = OFF , ALLOW_ROW_LOCKS = ON , ALLOW_PAGE_LOCKS = ON  )
	 ON [PRIMARY ] ;